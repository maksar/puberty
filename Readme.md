...
Next, let’s look at one of the possible ways of program execution – getting approval from the user with extended rights at any step of the process. The work begins with calling the `approve` function (for comparison here is a link to a corresponding [`ruby`](https://github.com/maksar/mentat/blob/master/version_6/workflow.rb#L10-L17) implementation). The function accepts two arguments: `User` and `Workflow` and always returns `Workflow`. If the user is inactive – `not (User.active user)`– then the result of the work is unchanged `workflow`. Otherwise, the scenario is pattern matching with the result of evaluating the expression `User.permission workflow.currentStep user` – calling the `permission` function from the `User` Package with arguments `workflow.currentStep` and `user`. It looks weird, but it helps the IDE, helpfully prompting the signature of the `permission` function: `permission : Int -> User -> Permission`. The `Permission` type is an algebraic sum, and the compiler requires of developers to clearly define what to do in each case.
...
Since the voting user has an extended set of rights, program execution will continue by calling the `increment` function. The signature of this function is the same as that one of `approve` – user comes first and `workflow` second. The choice of such an order of arguments is not accidental. The reason for this is that in `ELM`, as well as in other functional languages, `pipeline` operators `|>` and `<|` are widely used. They facilitate building chains of calls, when the returned value of a function is passed to the next. So, `increment`, using the auxiliary `step` function, constructs a new copy of `newWorkflow`, where the number of the step is increased by one. `newWorkflow`, in turn, is also a copy of `workflow` from the function’s arguments, but with a modified `votes` field.

In an imperative language, we could simply update one of the elements of this array. However, data in functional languages is usually immutable, so you cannot just add a vote to the many votes of the step. A new set based on the existing one, in which a new element is added, must be constructed. The `DictSet.insert user bucket` construction takes this task on, creating a new set based on the existing `bucket`, adding `user` to it. Note, that the creation of a new copy of the set does not lead to a change in the old one; the `bucket` array does not know anything about the created `bucket` copy either. So the array has to be modified too. This is done with the help of the `update` function from the `Array.Extra` package with the signature `update : Int -> (a -> a) -> Array a -> Array a`. The element index, which is subject to change, is passed as the first parameter; the array itself is passed last; in between them we have the function of element change. It’s such a function, where `bucket` will arrive to the input, and what happens at the output will be inserted into the array in its place. The returned value, of course, must be of the same type. 

The construction `let - in` may be a little hard to understand, but look at it in the following way: “let’s say `newWorkflow` is defined as `{ workflow | votes = ...` – then the entire `increment` can be expressed in terms `newWorkflow` as `step Forward newWorkflow`. So, in essence, this is just syntactic sugar, the explicit naming of the expression’s part.
...
Wow, feeling a little giddy from it all... But at least we managed to grasp the essence of `ELM`’s syntax and how `workflow` works. Now it will be much easier to understand testing.

## Traditional Testing

The test structure repeats the scenario in the image at the beginning of the article: `workflow` is created, consisting of three steps with thresholds 3, 2 and 2, respectively: `Workflow.init [ 3, 2, 2 ]`. Next, the operator `|>` takes the result of the expression on the left, that is, the initialized `workflow`, and inserts it as the last argument into the expression on the right. So you get: `approve (create "User 1" True [ VOTE ]) (Workflow.init [ 3, 2, 2 ])`, and this is precisely calling of the `approve` function with two arguments: `user` and `workflow`! Without using the pipe operator `|>` you would have to write a lengthy expression with an obscene amount of parentheses (hello, `lisp`). The logic of the tests themselves is seen from their descriptive part (the line after `test`). Before the final vote of “User 9”, `workflow` must be incomplete. It completes only after the final vote.
...
Well, so far we have not seen anything new as the test follows a standard structure: [arrange, act, assert](http://wiki.c2.com/?ArrangeActAssert). Let’s see what property-based testing can do.

## Property-based testing

To begin with, we need to formulate a number of invariants relative to `workflow`. Let’s take the first thing that comes to mind as a basis: the step number it is located at (`currentStep` property) should not be more than the number of steps, regardless of any actions users take with it. To achieve this you can use the array length of the `stepsConfig` property. After, for symmetry, we add another requirement: the non-negativity of the step number. Such an invariant in the code can be written as: `step >= 0 && step <= (length initWorkflow.stepsConfig)`. The condition `<=` is acceptable on the right boundary, since `workflow` transitioning to the completed state will formally appear in the next-to-last step. Now we have to determine what `step` is and how to express the concept “whatever users are doing” in the code.

To verify how `workflow` performs at the upper boundary, you can loosen up the statement “whatever users are doing” to “however many times different users `approve`” – since `reject` inevitably reduces the step number and `approve` only increases it. That is, if you have a list of voting users, then `step` can be expressed as `(foldl approve initWorkflow users).currentStep`. I agree it looks suspiciously short. I bet you can already imagine a cycle that goes through the user list and launches the voting function at each step. However, the functional nature of the language and careful approach to design methods simplify everything. `foldl` is a widely used higher `order` function that produces the [fold](https://en.wikipedia.org/wiki/Fold_(higher-order_function)) operation. Its three parameters are the conversion function `approve`, the initial value of `initWorkflow` and the list that you need to “fold” – `users`. Let me translate this for you: starting with `initialWorkflow`, successively call the `approve` function with arguments from the `users` list, in order to get the final `workflow`. Give yourself a pat on the back for choosing the right order of the `approve` function arguments, where the user is transferred as the first parameter, just like the “fold” contract requires.

Now we have everything to formulate the first property-based test. Notice how easily readable DSL is: claim (`claim`) that the hypothesis is true (`true`) for (`for`) a set of random `users`. Syntax `\(parameters) -> expression` (or `\parameter -> expression` in the case of one argument) is nothing else but a declaration of the anonymous function.
...
```
Before we delve into the essence of the expression `list userProducer`, let’s see the results of the launch. It is easy to see that a counterexample that refutes our invariant has been quickly found. I really wanted to believe in the code we wrote with such care, tested in the previous incarnation with the help of mutation tests, but it didn’t turn out to be so great. Looking at the counterexample, it becomes obvious what the problem is: if the user has `FORCE` privileges, the code does not do any checks and simply increases the number of the step!
